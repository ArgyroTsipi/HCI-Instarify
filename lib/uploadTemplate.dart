...
class uploadTemplate extends StatefulWidget {
  const uploadTemplate({Key? key}) : super(key: key);

  @override
  _uploadTemplateState createState() => _uploadTemplateState();
}
...
class _uploadTemplateState extends State<uploadTemplate> {
  ...
  Future getTemplate(ImageSource media) async {
    var img = await picker.pickImage(source: media);
  ...
  Widget build(BuildContext context) {
    ...
    if (!traversed) getTemplate(ImageSource.gallery);
    if (!traversed)
      return Scaffold();
    else
      return SafeArea(
